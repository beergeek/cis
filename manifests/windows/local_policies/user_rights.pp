...
#   }
#
# @param is_domain_controller Changes several settings if `true`. Adds or removes users/groups as per the standard. Affects Standard 2.2.2, 2.2.6, 2.2.17, 2.2.22, and 2.2.29.
...
# @param add_workstations_to_domain Users that can add a workstation to the domain. Only valid if `is_domain_controller` is `true`.
...
class cis::windows::local_policies::user_rights (
  ...
  Cis::Array_true $access_credential_manager      = ['No One'],
  Cis::Array_true $allow_computer_network_access  = ['Administrators','Authenticated Users'],
  Cis::Array_true $act_as_part_of_os              = ['No One'],
  Cis::Array_true $add_workstations_to_domain     = ['Administrators'],
  Cis::Array_true $adjust_process_memory_quote    = ['Administrators', 'LOCAL SERVICE', 'NETWORK SERVICE'],
  Cis::Array_true $allow_log_on_locally           = ['Administrators'],
  Cis::Array_true $allow_log_on_rdp               = ['Administrators','Remote Desktop Users'],
  Cis::Array_true $backup_users                   = ['Administrators'],
  Cis::Array_true $change_sys_time                = ['Administrators', 'LOCAL SERVICE'],
  Cis::Array_true $change_tz                      = ['Administrators', 'LOCAL SERVICE'],
  Cis::Array_true $change_pagefile                = ['Administrators'],
  Cis::Array_true $create_token_object            = ['No One'],
  Cis::Array_true $create_global_objects          = ['Administrators', 'LOCAL SERVICE', 'NETWORK SERVICE', 'SERVICE'],
  Cis::Array_true $create_perm_shared_objects     = ['No One'],
  Cis::Array_true $create_symbolic_links          = ['Administrators'],
  Cis::Array_true $debug_programs                 = ['Administrators'],
  Cis::Array_true $deny_computer_network_access   = ['Guests', 'Local account', 'member of Administrators group'],
  Cis::Array_true $deny_log_on_as_batch           = ['Guests'],
  Cis::Array_true $deny_log_on_as_service         = ['Guests'],
  Cis::Array_true $deny_local_log_on              = ['Guests'],
  Cis::Array_true $deny_log_on_rdp                = ['Guests','Local account'],
  Cis::Array_true $trusted_delegation             = ['No One'],
  Cis::Array_true $force_shutdwon_remote          = ['Administrators'],
  Cis::Array_true $gen_security_audits            = ['LOCAL SERVICE', 'NETWORK SERVICE'],
  Cis::Array_true $impersonate_client             = ['Administrators', 'LOCAL SERVICE', 'NETWORK SERVICE', 'SERVICE'],
  Cis::Array_true $increase_schedule_priority     = ['Administrators'],
  Cis::Array_true $manage_device_drivers          = ['Administrators'],
  Cis::Array_true $lock_pages_in_mem              = ['No One'],
  Cis::Array_true $log_on_as_batch                = ['Administrators',],
  Cis::Array_true $manage_audit_sec_logs          = ['Administrators'],
  Cis::Array_true $modify_object_label            = ['No One'],
  Cis::Array_true $mod_hw_env_values              = ['Administrators'],
  Cis::Array_true $perform_vol_maint              = ['Administrators'],
  Cis::Array_true $profile_single_proc            = ['Administrators'],
  Cis::Array_true $profile_sys_perf               = ['Administrators','NT SERVICE\WdiServiceHost'],
  Cis::Array_true $replace_proc_lvl_token         = ['LOCAL SERVICE', 'NETWORK SERVICE'],
  Cis::Array_true $restore_files_dirs             = ['Administrators'],
  Cis::Array_true $shutdown_sys                   = ['Administrators'],
  Cis::Array_true $sync_dir_service_data          = ['No One'],
  Cis::Array_true $own_files_and_objects          = ['Administrators'],
) {
  ...
  # CIS 2.2.1
  if $access_credential_manager != true {
  ...
  # CIS 2.2.2
  if $allow_computer_network_access != true {
  ...
  # CIS 2.2.4
  if $add_workstations_to_domain != true {
  ...
  # CIS 2.2.5
  if $adjust_process_memory_quote  != true {
  ...
  # CIS 2.2.14
  if $create_perm_shared_objects  != true {
  ...
  # CIS 2.2.17
  if $deny_computer_network_access  != true {
  ...
  # CIS 2.2.19
  if $deny_log_on_as_service  != true {
  ...
  if $deny_local_log_on  != true {
    ...
    # CIS 2.2.26
    if $increase_schedule_priority  != true {
    ...
    # CIS 2.2.29
    if $log_on_as_batch  != true and $enable_level_2 == true and $is_domain_controller == true {
    ...
    # CIS 2.2.36
    if $replace_proc_lvl_token  != true {
